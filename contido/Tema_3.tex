...
\begin{table}[H]
  ...
  \resizebox{15,0cm}{!}{
    \begin{tabular}{|c|c|c|}
      ...
      \hline  
      \multicolumn{1}{|p{5.0cm}|}{\centering El reparto anterior tuvo una violación de peso en la fase de verificación}& \multicolumn{1}{|p{10.0cm}|}{\centering \textbf{CRITICAR:} \\ Reparto.vehiculo.violacion = V\_Peso; \\ Reparto.Accion = Lista\_acciones;} & \multicolumn{1}{|p{4.0cm}|}{\centering Debido al tipo de violación, se devuelve una lista de acciones posibles que cumplen preferentemente los requisitos preferibles y que permiten dejar de cometer la violación. En este caso, las acciones están enfocadas a eliminar elementos que disparan la violación.}\\
      \hline  
      \multicolumn{1}{|p{5.0cm}|}{\centering Se selecciona de la lista de acciones una acción para ejecutar sobre el reparto.}& \multicolumn{1}{|p{10.0cm}|}{\centering \textbf{SELECCIONAR:} \\ Reparto.Acción = Acción\_Borrar; \\  } & \multicolumn{1}{|p{4.0cm}|}{\centering De la lista, se selecciona una acción de borrado.}\\
      \hline  
      \multicolumn{1}{|p{5.0cm}|}{\centering Seleccionada una acción, se aplica sobre el reparto.}& \multicolumn{1}{|p{10.0cm}|}{\centering \textbf{MODIFICAR:}: Accion\_Borrar.aplicar(Reparto.item\_seleccionado) ;} & \multicolumn{1}{|p{4.0cm}|}{\centering Se borra el elemento que ha violado los requisitos del reparto.}\\
      \hline  
    \end{tabular}
  }
...
\begin{lstlisting}[language=C,caption=\textbf{Regla\_de\_critica}]
  ...
  EXPRESSIONS:
    ...
    SI Reparto.item_seleccionado.tipo == normal
      Reparto.Accion = Accion_Mover_Normal;
      
    SI Reparto.item_seleccionado.tipo == frio
      Reparto.Accion = Accion_Mover_Frio;

    SI Reparto.item_seleccionado.tipo == animal
      Reparto.Accion = Accion_Mover_Animal;

    /* manisfestacion_reglas */

    SI Reparto.vehiculo.violacion != null TIENE-MANIFESTACION
      Reparto.Accion = Reparto.Item_reparto.Accion_Borrar;

    SI Reparto.item_seleccionado.tipo == Frio TIENE-MANIFESTACION
      Reparto.Accion = Accion_Mover_Frio;

    SI Reparto.item_seleccionado.tipo == Animal TIENE-MANIFESTACION
      Reparto.Accion = Accion_Mover_Animal;

    SI Reparto.item_seleccionado.tipo == Normal TIENE-MANIFESTACION
      Reparto.Accion = Accion_Mover_Normal;
